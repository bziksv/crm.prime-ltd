<div id="page-content" class="page-wrapper clearfix grid-button tickets-list-page">
    ...
    <div id="tickets-page-content" class="tickets-page-content view-table">
        <div class="tickets-list-pane">
            <div id="tickets-inbox-layout" class="tickets-inbox-layout">
                <aside class="tickets-inbox-sidebar">
                    <div class="tickets-inbox-toolbar">
                        <div class="tickets-inbox-toolbar-top">
                            <div class="tickets-inbox-search">
                                <i data-feather="search" class="icon-16"></i>
                                <input type="search" id="tickets-inbox-search" placeholder="Поиск: заголовок, клиент, домен, исполнитель..." autocomplete="off">
                            </div>
                        </div>
                        <div class="tickets-inbox-status-tabs">
                            <?php
                            $isClosed = isset($status) && $status === "closed";
                            ?>
                            <button type="button" data-status="open" class="<?php echo $isClosed ? '' : 'is-active'; ?>"><?php echo app_lang('open'); ?></button>
                            <button type="button" data-status="closed" class="<?php echo $isClosed ? 'is-active' : ''; ?>"><?php echo app_lang('closed'); ?></button>
                        </div>
                    </div>

                    <div id="tickets-inbox-list" class="tickets-inbox-list"></div>
                    <div id="tickets-inbox-load-more" class="tickets-inbox-load-more"></div>
                </aside>
            </div>

            <div class="tickets-table-view">
                <div class="card no-border-top-radius mb0">
                    <div class="table-responsive pb50">
                        <table id="ticket-table" class="display" cellspacing="0" width="100%"></table>
                    </div>
                </div>
            </div>
        </div>

        <section id="tickets-detail-pane" class="tickets-detail-pane">
            <div id="ticket-side-panel-body" class="ticket-side-panel-body">
                <div class="ticket-side-panel-placeholder">Выберите заявку из списка</div>
            </div>
        </section>
    </div>
...
<link rel="stylesheet" href="<?php echo base_url('assets/css/tickets-inbox.css?v=20260829a'); ?>">
...
<script src="<?php echo base_url('assets/js/tickets-panel.js?v=20260828p'); ?>"></script>
...
<?php } ?>
        ...
        filterDropdowns.push({name: "status", class: "w200", options: [
                ...
                {id: 'open', text: '<?php echo app_lang("open") ?>', isSelected: selectOpenStatus},

                {id: 'closed', text: '<?php echo app_lang("closed") ?>', isSelected: selectClosedStatus},
        ...
        window.ticketTableRestoreConfig = {

            hiddenIndexes: [0, 7, 9],

            hideProjectColumn: !projectVisibility,

            hideOptionsColumn: !optionsVisibility
        ...
        $("#ticket-table").appTable({
            ...
            smartFilterIdentity: "tickets_list",
            ...
            rowCallback: function (nRow, aData) {
                ...
                if (window.TicketSidePanel && TicketSidePanel.setRowCallback) {

                    TicketSidePanel.setRowCallback(nRow, aData);
            ...
            onInitComplete: function () {
                if (window.TicketSidePanel && TicketSidePanel.restoreColumns) {
                    TicketSidePanel.restoreColumns();
                }
            }
