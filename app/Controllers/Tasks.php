...
class Tasks extends Security_Controller {
    ...
    function delete() {
        ...
        if ($this->Tasks_model->delete_task_and_sub_items($id)) {
            ...
            //log the task deletion in the activity logs (deletion goes through raw SQL and skips Crud_model logging)
            $this->Activity_logs_model->ci_save(array(
                "action" => "deleted",
                "log_type" => "task",
                "log_type_title" => $info->title ? $info->title : "",
                "log_type_id" => $id,
                "log_for" => "project",
                "log_for_id" => $info->project_id,
            ));
    ...
    function update_checklist_item()
    {
        ...
        if ($checklist_title) {
            ...
            $task_id = $this->Checklist_items_model->get_one($checklist_id)->task_id;
            $task_info = $this->Tasks_model->get_one($task_id);
            $this->_save_checklist_activity_log($task_info, "checklist_item_updated", $checklist_title);
        }
    ...
    private function _save_checklist_activity_log($task_info, $action, $title) {
        ...
        $this->Activity_logs_model->ci_save(array(
            ...
            "changes" => serialize(array("checklist_item" => $title)),
            "log_for" => "project",
        ...
        if ($is_checklist_group) {
            ...
            foreach ($checklists as $checklist) {
                ...
                if ($save_id) {
                    ...
                    $this->_save_checklist_activity_log($task_info, "checklist_item_added", $item_info->title);
                }
            }
        } else {
            $data = array(
    ...
    function save_checklist_item_status($id = 0) {
        ...
        if ($save_id) {
            $item_info = $this->Checklist_items_model->get_details(array("id" => $save_id))->getRow();
            $status_action = $item_info->is_checked ? "checklist_item_checked" : "checklist_item_unchecked";
            $this->_save_checklist_activity_log($task_info, $status_action, $item_info->title);
    ...
    function delete_checklist_item($id) {
        ...
        $item = $this->Checklist_items_model->get_one($id);
        ...
        if ($this->Checklist_items_model->delete($id)) {
            $task_info = $this->Tasks_model->get_one($task_id);
            $this->_save_checklist_activity_log($task_info, "checklist_item_deleted", $item->title);
            echo json_encode(array("success" => true));
        } else {
            echo json_encode(array("success" => false));
        }
